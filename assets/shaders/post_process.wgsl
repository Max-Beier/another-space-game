...
fn fragment(in: FullscreenVertexOutput) -> @location(0) vec4<f32> {
    let v = view.inverse_view[0].xyz;
    let color = vec4<f32>(
        textureSample(screen_texture, texture_sampler, in.uv).rgb,
        1.0
    );

    let ray_origin = view.inverse_view[3].xyz;
    let ray_direction = normalize(in.position.xyz - ray_origin);

    let hit_info = ray_sphere(atmosphere.center, atmosphere.atmosphere_radius, ray_origin, ray_direction);

    let dst_to_atmosphere = hit_info.x;
    let dst_through_atmosphere = hit_info.y;

    let res = dst_through_atmosphere / (atmosphere.atmosphere_radius * 2.0);

    return color - vec4<f32>(res) ;
}
...
fn density_at_point(density_sample_point: vec3<f32>) -> f32 {
    let falloff = 13.0;
    let height_above_surface = length(density_sample_point - atmosphere.center) - (atmosphere.atmosphere_radius - 100.0);
    let height01 = height_above_surface / atmosphere.atmosphere_radius;
...
fn calculate_light(ray_origin: vec3<f32>, ray_direction: vec3<f32>, ray_length: f32) -> vec3<f32> {
    ...
    for (var i: i32 = 0; i < num_in_scattering_points; i = i + 1) {
        let sun_ray_length = ray_sphere(atmosphere.center, atmosphere.atmosphere_radius, in_scatter_point, atmosphere.light_dir).y;
        let sun_ray_optical_depth = optical_depth(in_scatter_point, atmosphere.light_dir, sun_ray_length);
